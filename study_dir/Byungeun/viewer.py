...
class Listen(QThread):
    def __init__(self, display):
        ...
        self.access_key = "F59AapKj7fhOtu3RyzJ4yKWXZPYX9jyLdu8B9nPCztR2zQj6j0VKYQ==" #리눅스용
    ...
    def init_porcupine_and_recorder(self):
        ...
        self.recorder = PvRecorder(frame_length=self.porcupine.frame_length, device_index=2)
        self.recorder.start()
    ...
    def obj_tracking(self):
        self.ssd_tracking = threading.Thread(target=cracker.detect_objects_and_poses)
        self.ssd_tracking.start()


    def start_stt_process(self):
        from STT import SpeechToText, KeywordFinder
        self.display.stt = SpeechToText(on_complete=self.restart_recorder)
        self.display.keyword_finder = KeywordFinder()
        self.display.stt_thread = threading.Thread(target=self.display.stt.continuous_listen, args=(self.display.keyword_finder,))
        self.display.stt_thread.start()
        
    def restart_recorder(self):
        """ STT 처리가 끝난 후 레코더를 다시 시작합니다. """
        self.init_porcupine_and_recorder()
